...
class Jogo {
  ...
  aumentaPontos(time) {
    //time é um número 0 ou 1 informado no onclick do aumento de pontuação
    ...
    if (basquete.times.length != 0) {
      ...
      for (let i = 0; i < basquete.jogos.length; i++) {
        ...
        button1.setAttribute(
          'onclick',
          "vencer('basquete'," + i + ',basquete.jogos[' + i + '].times[0].nome)'
        );
        button2.setAttribute(
          'onclick',
          "vencer('basquete'," + i + ',basquete.jogos[' + i + '].times[1].nome)'
        );
